...
import { LayoutDashboard, List, TrendingUp, MessageSquare, Settings, Plus, LogOut } from "lucide-react";
...
export default function AdminSidebar() {
  ...
  return (
    <aside className="h-screen w-64 border-r border-zinc-200 bg-zinc-50 flex flex-col p-4 gap-2 flex-shrink-0 hidden md:flex">
      <div className="mb-8 mt-4 px-2">
        <h1 className="text-lg font-black text-emerald-900 tracking-tighter uppercase">Hulu Properties</h1>
        <p className="text-zinc-500 mt-1 text-xs uppercase tracking-widest font-semibold">Admin Console</p>
      </div>
      <nav className="flex flex-col gap-2 flex-1">
        {links.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${isActive ? "bg-emerald-900/10 text-emerald-900 font-semibold" : "text-zinc-600 hover:bg-zinc-200 hover:text-zinc-900"}`}>
              <Icon size={20} />
